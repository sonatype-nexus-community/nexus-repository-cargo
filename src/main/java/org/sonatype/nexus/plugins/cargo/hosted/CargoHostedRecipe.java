...
class CargoHostedRecipe extends CargoRecipeSupport {
    ...
    private ViewFacet configure(final ConfigurableViewFacet facet) {
        ...
        builder.route(new Route.Builder()
                .matcher(new TokenMatcher("/{repo_name:index}/info/refs"))
                ...
                .handler(gitInfoRefsHandler)
                .create());

        // Git client fetching objects over Smart HTTP protocol
        builder.route(new Route.Builder()
                .matcher(new TokenMatcher("/{repo_name:index}/git-upload-pack"))
                .handler(timingHandler)
                .handler(securityHandler)
                .handler(exceptionHandler)
                .handler(conditionalRequestHandler)
                .handler(contentHeadersHandler)
                .handler(unitOfWorkHandler)
                .handler(gitUploadPackHandler)
                .create());

        // Git client pushing objects over Smart HTTP protocol
        builder.route(new Route.Builder()
                .matcher(new TokenMatcher("/{repo_name:index}/git-receive-pack"))
                .handler(timingHandler)
                .handler(securityHandler)
                .handler(exceptionHandler)
                .handler(conditionalRequestHandler)
        ...
        builder.route(new Route.Builder()
                .matcher(LogicMatchers.and(
                        new ActionMatcher(HttpMethods.DELETE),
                        new LiteralMatcher("/token")))
                .handler(timingHandler)
                .handler(securityHandler)
                .handler(exceptionHandler)
        ...
        builder.route(new Route.Builder()
                ...
                .handler(contentHeadersHandler)
                .handler(unitOfWorkHandler)
                .handler(metadataDownloadHandler)
                .create());
