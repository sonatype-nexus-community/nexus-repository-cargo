...
public class GitRepositoryHandlers {
    ...
    private final static String[] GET_REQUEST_METHODS = {
            HttpMethods.GET, HttpMethods.HEAD
    };
    private final static String[] PUT_REQUEST_METHODS = {
            HttpMethods.POST
    };

    private static abstract class GitHandlerSupport extends ComponentSupport implements Handler {
        ...
        protected static Response setUncachedResponse(Response response) {
            return new Response.Builder()
                    .copy(response)
                    .header(HttpHeaders.EXPIRES, "Fri, 01 Jan 1980 00:00:00 GMT")
        ...
        protected static Response buildBadRequestResponse(String[] allowed_methods) {
            return setUncachedResponse(new Response.Builder()
                    .copy(HttpResponses.badRequest())
                    .header(HttpHeaders.ALLOW, String.join(", ", allowed_methods))
                    .build());
        };
    }

    @Named
    public static class Default extends GitHandlerSupport {
        @Override
    ...
    @Named
    public static class InfoRefs extends GitHandlerSupport {
        @Override
        public Response handle(Context context) throws Exception {
        ...
        public Response handleSmartUploadPackService(Context context) throws Exception {
            ...
            service.sendAdvertisedRefs(
                    new RefAdvertiser.PacketLineOutRefAdvertiser(packet_line_out));

            Payload payload = new BytesPayload(out_bytes.toByteArray(),
                    UPLOAD_PACK_ADVERTISEMENT_TYPE);
        ...
        public Response handleSmartReceivePackService(Context context) throws Exception {
            ...
            Payload payload = new BytesPayload(out_bytes.toByteArray(),
                    RECEIVE_PACK_ADVERTISEMENT_TYPE);
            return HttpResponses.ok(payload);
        }
    ...
    @Named
    public static class UploadPackService extends GitHandlerSupport {
        @Override
        @TransactionalTouchMetadata
        @TransactionalTouchBlob
        public Response handle(Context context) throws Exception {
            if (!new ActionMatcher(PUT_REQUEST_METHODS).matches(context)) {
                this.createLogger().debug("Wrong upload-pack action: {}",
                        context.getRequest());
                return buildBadRequestResponse(PUT_REQUEST_METHODS);
            }

            if (!context.getRequest().getPayload().getContentType()
                    .equals(UPLOAD_PACK_REQUEST_TYPE)) {
            ...
            Payload payload = new BytesPayload(
                    outputBytes.toByteArray(), UPLOAD_PACK_RESULT_TYPE);
            return HttpResponses.ok(payload);
        }
    };

    @Named
    public static class ReceivePackService extends GitHandlerSupport {
        ...
        public Response handle(Context context) throws Exception {
            if (!new ActionMatcher(PUT_REQUEST_METHODS).matches(context)) {
                this.createLogger().debug("Wrong receive-pack action: {}",
                        context.getRequest());
                return buildBadRequestResponse(PUT_REQUEST_METHODS);
            }

            if (!context.getRequest().getPayload().getContentType()
                    .equals(RECEIVE_PACK_REQUEST_TYPE)) {
            ...
            Payload payload = new BytesPayload(
                    outputBytes.toByteArray(), RECEIVE_PACK_RESULT_TYPE);
            return HttpResponses.ok(payload);
        }
    };
