...
@Named
public class CargoRegistryFacetImpl extends FacetSupport implements CargoRegistryFacet {
    ...
    static class Config {
        @NotNull(groups = {
                HostedType.ValidationGroup.class
        })
    ...
    protected void doConfigure(final Configuration configuration) throws Exception {
        config = facet(ConfigurationFacet.class).readSection(configuration, CONFIG_KEY,
                Config.class);
    }
    ...
    public void writeConfigJson() throws Exception {
        ...
        Repository indexRepo = this.getRepository()
                .facet(GitRepositoryFacet.class).getGitRepository("index");
        ...
        if (headId != null) {
            try (RevWalk revWalk = new RevWalk(indexRepo)) {
                ...
                builder.addTree(null, DirCacheEntry.STAGE_0, indexRepo.newObjectReader(),
                        parent.getTree());
                builder.finish();
            }
        }
    ...
    public Response publishCrate(CrateCoordinates crateId, JsonElement metadata,
        ...
        Component crateComponent = this.crateAttributes.find(this.getRepository(),
                crateId);
