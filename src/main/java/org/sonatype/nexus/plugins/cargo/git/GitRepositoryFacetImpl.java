...
@Named
public class GitRepositoryFacetImpl extends FacetSupport implements GitRepositoryFacet {
    ...
    public Repository createGitRepository(String repo_name) throws IOException {
        Preconditions.checkNotNull(repo_name,
                "getGitRepository called without a 'repo_name' token");
        Component component = this.component_attributes.createGitComponent(this.getRepository(),
                repo_name);
        ...
        Repository repo = this.builder
                .setStorageFacet(storage_facet)
                .setComponent(bucket, component)
                .build();
    ...
    public Repository getGitRepository(String repo_name) throws IOException {
        ...
        Component component = this.component_attributes.findGitComponent(
                this.getRepository(),
                repo_name);
        if (component == null) {
            return null;
    ...
    public void replaceFile(Repository repository, String branch, String entryPath, byte[] bytes)
        ...
        if (headId != null) {
            try (RevWalk revWalk = new RevWalk(repository)) {
                ...
                builder.addTree(null, DirCacheEntry.STAGE_0, repository.newObjectReader(),
                        parent.getTree());
                builder.finish();
            }
        }
        ...
        PersonIdent adminIdent = new PersonIdent("Nexus System",
                this.emailManager.getConfiguration().getFromAddress());
