...
public final class CargoRegistryV1Handlers {
    ...
    private static abstract class CrateHandlerSupport extends ComponentSupport implements Handler {
    ...
    @Named
    public static class Publish extends CrateHandlerSupport {
        ...
        public Response handle(Context context) throws Exception {
            ...
            LittleEndianDataInputStream requestBody = new LittleEndianDataInputStream(
                    context.getRequest().getPayload().openInputStream());
            try {
                ...
                try {
                    ...
                    JsonElement json = jsonParser
                            .parse(new InputStreamReader(new ByteArrayInputStream(jsonBytes)));
                    if (!json.isJsonObject()) {
                        return HttpResponses
                                .badRequest("Expected JSON portion to contain an object");
                    }
     ...
     */
    @Named
    public static class CrateDownload extends CrateHandlerSupport {
        @Inject
        ...
        public Response handle(@Nonnull final Context context) throws Exception {
            ...
            Preconditions.checkNotNull(version,
                    "Download handler called without a 'version' token");
     ...
     */
    @Named
    public static class MetadataDownload extends CrateHandlerSupport {
        @Inject
    ...
    @Named
    public static class IndexInfoRefs extends GitRepositoryHandlers.InfoRefs {
        @Override
        public Response handle(Context context) throws Exception {
    ...
    @Named
    public static class IndexRecievePackService extends GitRepositoryHandlers.ReceivePackService {
        @Inject
    ...
    @Named
    public static class IndexUploadPackService extends GitRepositoryHandlers.UploadPackService {
        @Override
        public Response handle(Context context) throws Exception {
            context.getRepository().facet(CargoRegistryFacet.class).writeConfigJson();
            return super.handle(context);
